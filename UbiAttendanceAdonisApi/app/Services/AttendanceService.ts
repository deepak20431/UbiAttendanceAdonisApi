...
export default class AttendanceService {
  ...
  static async EmployeesList(data) {
    ...
    const query = await Database.from("EmployeeMaster")
      .select(
        Database.raw(
          "Id,OrganizationId, CONCAT(FirstName, ' ', LastName) as name, EmployeeCode as ecode, NotificationStatus "
        )
      )
      .where("archive", 1)
  ...
  static async Service_setQrKioskPin(Data) {
    ...
    if (query.length > 0) {
      const query2 = await Database.from("UserMaster")
      .select("*")
      .where("EmployeeId", Data.empid)
      .andWhere("OrganizationId", Data.orgid)
  ...
  static async getSetKioskPinService(Data) {
    const result: any = {};
    const query1 = await Database.from("UserMaster")
      .select("kioskPin")
      .where("EmployeeId", Data.empId)
      .andWhere("OrganizationId", Data.orgId);
